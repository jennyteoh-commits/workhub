...
const I18N = {
  en: {
    ...
    mthOtherOpen: 'Other',
    lblAllYears: 'All Years',
    lblAllMonths: 'All Months',
    lblAllTime: 'All Time',
    lblAllTab: 'All',
...
let appState = {
  ...
  filters: {
    ...
    // Dashboard Year / Month filter — controls every Dashboard section
    // (KPIs, Monthly Task Summary, donut chart, Recent Tasks). Defaults to
    // the current year/month, computed fresh at load time; either can be
    // switched to 'ALL' independently. Never persisted/hardcoded.
    dashboardYear: getTodaySGTStr().slice(0, 4),
    dashboardMonthNum: getTodaySGTStr().slice(5, 7)
  },
...
function updateInterfaceLanguage() {
  ...
  // Dashboard Recent Tasks Table Headers (9 columns, incl. Completion Date)
  const recHeaders = document.querySelectorAll('#recentTasksTableBody').length
    ? document.getElementById('recentTasksTableBody').closest('table').querySelectorAll('thead th')
    : [];
  if (recHeaders && recHeaders.length >= 9) {
    ...
    if (recHeaders[7]) recHeaders[7].textContent = t('thCompletionDate');
    if (recHeaders[8]) recHeaders[8].textContent = t('thActions');
  }
...
function renderDashboard() {
  renderDashboardDateFilterOptions();

  const allTasks = appState.tasks;
  const selectedProj = appState.filters.dashboardProject || 'ALL';
  const projTasks = selectedProj === 'ALL' ? allTasks : allTasks.filter(t => (t.project || 'SG Project') === selectedProj);

  const selYear = appState.filters.dashboardYear;
  const selMonth = appState.filters.dashboardMonthNum;

  // FULL-YEAR scope — drives ONLY the top 5 KPI cards. Controlled by Year +
  // Project; the Month selector must NOT narrow these (per spec: switching
  // month must not change the 5 KPI cards, only switching year does).
  let yearTasks = projTasks;
  if (selYear && selYear !== 'ALL') {
    yearTasks = yearTasks.filter(t => t.dueDate && t.dueDate.slice(0, 4) === selYear);
  }

  // YEAR + MONTH scope — drives Monthly Task Summary, its donut chart, and
  // Recent Tasks (unchanged from before: Year + Month + Project all apply).
  let tasks = yearTasks;
  if (selMonth && selMonth !== 'ALL') {
    tasks = tasks.filter(t => t.dueDate && t.dueDate.slice(5, 7) === selMonth);
  }

  const todayStr = getTodaySGTStr();

  // Calculate KPI Counts — from the FULL-YEAR scope only (Month selector
  // does not affect these 5 cards).
  ...
  const openTasksCount = yearTasks.filter(t => t.status !== 'Completed' && t.status !== 'Cancelled').length;
  const completedTasksCount = yearTasks.filter(t => t.status === 'Completed').length;
  const dueTodayCount = yearTasks.filter(t => t.dueDate === todayStr && t.status !== 'Completed' && t.status !== 'Cancelled').length;
  const overdueCount = yearTasks.filter(t => t.dueDate < todayStr && t.status !== 'Completed' && t.status !== 'Cancelled').length;

  // Record Today's KPI Snapshot into Local History only when viewing the
  // fully unfiltered scope that actually drives the KPI cards (all projects,
  // all years) — Month is excluded from this check since it no longer
  // affects the KPI cards at all. The "change since yesterday" comparison
  // only makes sense against that same scope, so a narrower Year/Project
  // selection shows "no comparison yet" instead of a misleading number (see
  // updateKpiCard's null-diff case).
  const isFullyUnfiltered = selectedProj === 'ALL' && selYear === 'ALL';
  ...
  const yesterdaySnapshot = isFullyUnfiltered ? getYesterdayKpiSnapshot() : null;
  ...
  // Render Charts (Tasks by Status doughnut) — Year + Month + Project scope, unchanged
  renderDashboardCharts(tasks);

  // Render Monthly Task Summary (Year + Month + Project scope)
...
// Shared, single-source-of-truth task classification used by both the
// Monthly Task Summary cards and its donut chart, so "Completed" / "Overdue"
// / "In Progress" / "Waiting" always mean the same thing in both places and
// a task is never counted in more than one bucket. Priority order (a task
// is placed in the FIRST bucket it matches):
//   1. Completed   — status === 'Completed'
//   2. Overdue     — not Completed/Cancelled, and Due Date is before today
//   3. In Progress — status === 'In Progress' (and not already Overdue)
//   4. Waiting     — status === 'Waiting' (and not already Overdue)
//   5. Other       — anything else (Not Started, Review, On Hold, Cancelled,
//                    or any future status not covered above)
function classifyTaskBucket(task, todayStr) {
  if (task.status === 'Completed') return 'completed';
  if (task.status !== 'Cancelled' && task.dueDate && task.dueDate < todayStr) return 'overdue';
...
// ==========================================================================
// Builds the human label for the Dashboard's current Year/Month filter
// selection, e.g. "August 2026", "2026 (All Months)", "August (All Years)",
// or "All Time" — shown as the Monthly Task Summary's subtitle.
function getDashboardFilterLabel() {
  const y = appState.filters.dashboardYear;
  const m = appState.filters.dashboardMonthNum;
  const monthName = (mm) => {
    const idx = parseInt(mm, 10) - 1;
    return appState.lang === 'zh' ? MONTH_NAMES_ZH[idx] : MONTH_NAMES_EN[idx];
  };
  if (y === 'ALL' && m === 'ALL') return t('lblAllTime');
  if (y === 'ALL') return `${monthName(m)} (${t('lblAllYears')})`;
  if (m === 'ALL') return `${y} (${t('lblAllMonths')})`;
  return `${monthName(m)} ${y}`;
}

// Populates the Dashboard's Year / Month <select> filter options dynamically
// from the actual task Due Dates present (plus the current year, so the
// dropdown is never empty on a brand new dataset). Never hardcoded.
function renderDashboardDateFilterOptions() {
  ...
  const currentYear = getTodaySGTStr().slice(0, 4);
  const yearsSet = new Set([currentYear]);
  appState.tasks.forEach(task => {
    if (task.dueDate && task.dueDate.length >= 4) yearsSet.add(task.dueDate.slice(0, 4));
  });
  const years = Array.from(yearsSet).sort();

  if (appState.filters.dashboardYear !== 'ALL' && !years.includes(appState.filters.dashboardYear)) {
    appState.filters.dashboardYear = currentYear;
  }

  yearSelect.innerHTML = `<option value="ALL" ${appState.filters.dashboardYear === 'ALL' ? 'selected' : ''}>${t('lblAllYears')}</option>` +
    years.map(y => `<option value="${y}" ${appState.filters.dashboardYear === y ? 'selected' : ''}>${y}</option>`).join('');

  monthSelect.innerHTML = `<option value="ALL" ${appState.filters.dashboardMonthNum === 'ALL' ? 'selected' : ''}>${t('lblAllMonths')}</option>` +
    MONTH_NAMES_EN.map((_, idx) => {
      const val = String(idx + 1).padStart(2, '0');
      const label = appState.lang === 'zh' ? MONTH_NAMES_ZH[idx] : MONTH_NAMES_EN[idx];
      return `<option value="${val}" ${appState.filters.dashboardMonthNum === val ? 'selected' : ''}>${label}</option>`;
    }).join('');
}
...
function renderMonthlyTaskSummary(tasks) {
  ...
  const todayStr = getTodaySGTStr();
  // `tasks` arrives already filtered by the Dashboard's Project + Year/Month
  // filter (see renderDashboard()) — this card just summarizes that set.
  const monthTasks = tasks;

  const totalTasks = monthTasks.length;

  // Mutually-exclusive bucket counts, shared with the donut chart below via
  // classifyTaskBucket() so the two never disagree and always reconcile.
  const buckets = { completed: 0, overdue: 0, inProgress: 0, waiting: 0, other: 0 };
  monthTasks.forEach(task => { buckets[classifyTaskBucket(task, todayStr)]++; });
  ...
  const otherOpen = buckets.other;
  const completionRate = totalTasks > 0 ? Math.round((completed / totalTasks) * 100) : 0;

  if (summarySubEl) summarySubEl.textContent = getDashboardFilterLabel();
  ...
  // Donut Chart — 5 mutually-exclusive categories (Completed, Overdue, In
  // Progress, Waiting, and Other, the last only shown when it has tasks in
  // it), built from the SAME bucket counts as the cards above so the donut
  // total always reconciles with Total Tasks.
  ...
  if (chartCanvas && typeof Chart !== 'undefined') {
    ...
    const donutLabels = [t('mthCompleted'), t('mthOverdue'), t('mthInProgress'), t('mthWaiting')];
    const donutData = [completed, overdue, inProgress, waiting];
    // Overdue reuses #FFAA93 — the same color already used for overdue Due
    // Date text elsewhere in the app (e.g. Recent Tasks, Task Management).
    const donutColors = [STATUS_COLOR_MAP['Completed'], '#FFAA93', STATUS_COLOR_MAP['In Progress'], STATUS_COLOR_MAP['Waiting']];
    if (otherOpen > 0) {
      donutLabels.push(t('mthOtherOpen'));
      donutData.push(otherOpen);
      donutColors.push('#CFCFCF');
    }
...
function renderRecentTasksTable(tasks) {
  ...
  if (displayTasks.length === 0) {
    tbody.innerHTML = `
      <tr>
        <td colspan="9" style="text-align: center; padding: 32px; color: var(--text-muted); font-weight: 700;">
          ${t('emptyTable')}
        </td>
      </tr>
    `;
...
function renderTaskMonthTabs() {
  ...
  // current month rather than leaving the UI on a dead selection. 'ALL' is
  // always a valid selection and is exempt from this guard.
  if (appState.filters.taskMonth !== 'ALL' && !months.includes(appState.filters.taskMonth)) {
    appState.filters.taskMonth = getTodaySGTStr().slice(0, 7);
  }

  const allTabHtml = `
    <button type="button" class="month-tab ${appState.filters.taskMonth === 'ALL' ? 'active' : ''}" onclick="selectTaskMonth('ALL')">
      ${t('lblAllTab') || 'All'}
    </button>
  `;

  container.innerHTML = allTabHtml + months.map(m => {
...
function renderTaskManagement() {
  ...
  let filtered = appState.tasks.filter(task => {
    ...
    // Month Tab Filter (by Due Date's year-month). 'ALL' bypasses this check
    // entirely, showing tasks across every month/year (still respecting the
    // Project/Status/Priority/Category/Search filters above).
    if (appState.filters.taskMonth && appState.filters.taskMonth !== 'ALL' &&
        (!task.dueDate || task.dueDate.slice(0, 7) !== appState.filters.taskMonth)) {
      return false;
    }
  ...
  // Sort Tasks (per the selected sort option)
  filtered.sort((a, b) => {
  ...
  // Completed-last display sorting rule: non-completed tasks always appear
  // above Completed tasks, regardless of the selected sort option above.
  // Within non-completed tasks, the selected sort order above is preserved.
  // Within Completed tasks (moved to the bottom), sort by Due Date nearest-first.
  // This is a DISPLAY-ONLY re-partition — it does not mutate task data, IDs,
  // statuses, or due dates in any way.
  const nonCompleted = filtered.filter(t => t.status !== 'Completed');
  const completedOnly = filtered.filter(t => t.status === 'Completed')
    .sort((a, b) => a.dueDate.localeCompare(b.dueDate));
  filtered = nonCompleted.concat(completedOnly);
...
function initTaskFilterListeners() {
  ...
  const dashYearFilter = document.getElementById('dashboard-filter-year');
  if (dashYearFilter) {
    dashYearFilter.addEventListener('change', (e) => {
      appState.filters.dashboardYear = e.target.value;
      renderDashboard();
      if (window.lucide) lucide.createIcons();
    });
  }

  const dashMonthFilter = document.getElementById('dashboard-filter-month');
  if (dashMonthFilter) {
    dashMonthFilter.addEventListener('change', (e) => {
      appState.filters.dashboardMonthNum = e.target.value;
      renderDashboard();
      if (window.lucide) lucide.createIcons();
    });
  }
