...
function updateInterfaceLanguage() {
  ...
  // Task Management Table Headers (9 columns)
  const taskHeaders = document.querySelectorAll('#page-tasks .data-table thead th');
  if (taskHeaders && taskHeaders.length >= 9) {
    ...
    if (taskHeaders[7]) taskHeaders[7].textContent = t('thAssignee');
    if (taskHeaders[8]) taskHeaders[8].textContent = t('thActions');
  }
...
function renderTaskManagement() {
  ...
  if (filtered.length === 0) {
    tbody.innerHTML = `
      <tr>
        <td colspan="9" style="text-align: center; padding: 40px; color: var(--text-muted); font-weight: 700;">
...
function attemptSilentGoogleReconnect() {
  if (isGoogleTokenValid() || appState.google.autoConnectInProgress) return;
...
// (Start Time/End Time/Completed Date/Created At/Updated At/Google Event ID
// on Tasks; Start Date/End Date/Created At/Updated At on Projects; Created
// At/Updated At on Categories) is sent as an empty string rather than
// invented — the Apps Script leaves those Sheet columns blank.
...
async function deleteTaskFromGoogleSheets(taskId) {
  if (!isGoogleSheetsConnected()) return { ok: false, skipped: true };
  try {
    await callGoogleSheetsApi('deleteTask', { id: taskId });
    return { ok: true };
  } catch (err) {
    handleGoogleSheetsApiError(err);
    return { ok: false, error: err };
...
async function runFullGoogleSheetsSync(label) {
  ...
  for (const taskItem of tasks) {
    const r = await syncTaskToGoogleSheets(taskItem, label);
    if (r.ok) done.tasks++; else failed.tasks++;
    updateGoogleSheetsSyncProgressUI(label, done, totals, 'running');
  }
  for (const projectItem of projects) {
    const r = await syncProjectToGoogleSheets(projectItem, label);
    if (r.ok) done.projects++; else failed.projects++;
    updateGoogleSheetsSyncProgressUI(label, done, totals, 'running');
  }
  for (const categoryItem of categories) {
    const r = await syncCategoryToGoogleSheets(categoryItem, label);
  ...
  // Only stamp "Last Synced" when nothing failed outright due to a broken
  // connection (individual per-record failures still count as an attempt —
  // Sync Now can simply be run again, since it's a safe UPSERT of everything).
